...
PUBLIC int DobbyInstrumentQBDI(void *address, dobby_instrument_callback_t_QBDI pre_handler) {
  if (!address) {
  ...
  auto routing = new InstructionInstrumentRoutingQBDI(entry, pre_handler, nullptr);
  routing->Prepare();
